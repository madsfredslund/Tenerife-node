...
def findshift(im0,im1):
	...
	image_product = numpy.fft.fft2(im0) * numpy.fft.fft2(im1).conj()
	...
	##### in integer pixels
	x_off = (x[0] - cc_image.real.shape[1]/2)
	y_off = (y[0] - cc_image.real.shape[0]/2)
	#####
 ...
 #	y = numpy.array(y.tolist())
	...
	coeffi = [1., float(y), 20.0]


	new_x1, new_y1, x_max, bla = fit_with_gauss(range(len(cc_image.real[x,:])), (numpy.array(cc_image.real[x,:]) - numpy.min(cc_image.real[x,:])) / (numpy.max(cc_image.real[x,:] - numpy.min(cc_image.real[x,:]))), coeffi)

	coeffi = [1., float(x), 20.0]
	new_x2, new_y2, y_max, bla = fit_with_gauss(range(len(cc_image.real[:,y])), (numpy.transpose(cc_image.real[:,y]) - numpy.min(numpy.transpose(cc_image.real[:,y]))) / (numpy.max(numpy.transpose(cc_image.real[:,y]))- numpy.min(numpy.transpose(cc_image.real[:,y]))), coeffi)

#	plt.plot(cc_image.real[x,:])
...
#		y_max -= Y
	...
	x_max= (x_max - cc_image.real.shape[1]/2)
	y_max = (y_max - cc_image.real.shape[0]/2)
