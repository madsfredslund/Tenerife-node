...
class Checker(object):
   ...
   def get_fields_time_int(self, curr, table_name, fields=[], time_int=0):
      ...
      stmt = "SELECT %s FROM %s WHERE ins_at > (current_timestamp - INTERVAL '%s minutes')" % (field_str, table_name, int(time_int))
      curr.execute(stmt)
   ...
   def weather_check(self, deduced=0):
      ...
      try:
		conn = psycopg2.connect("host='%s' dbname='%s' user='%s' password='%s'" % (m_conf.db_host, m_conf.data_db, m_conf.db_user, m_conf.db_password))
		curr = conn.cursor()
		output_dome = self.get_fields(curr, "tel_dome", ["tel_az", "dome_az", "ins_at", "dome_slit_state"])
      except Exception, e:
      ...
      elif song_checker_config.use_bw_for_wind == "yes" and float(weather_output["bw_windspeed"]) >= song_checker_config.max_w_speed_into:
         weather_value = weather_value + 4
	...
	# if sun_alt < 0.0 or float(output_dome["dome_slit_state"]) > 0.0:
#		 print clock.timename(), "The wind speed into the dome was too high. Wind dir = %s, Telescope az = %s" % (str(float(weather_output["wxt520_wind_avg"])), str(float(output_dome['tel_az']))) 
	...
	# Boltwood cloud detector, 0 = unknown, 1 = clear, 2 = cloudy, 3 = very cloudy.
      ...
      for i in song_checker_config.clouds:
		if i == 3:
			...
			cloud_events = len(numpy.where(numpy.array(boltwood_cloud_arr) >= i)[0])
			if cloud_events > (len(numpy.array(boltwood_cloud_arr)) * 0.75):	# if there has been more than 3/4 of the events in the last 20 minutes 
				if weather_value < 16:			
					weather_value = weather_value + 16
	...
	###### IF ONLY ONE TRIGGERS IT WILL CHECK IF MORE THAN ONE EVENT OCCURED ######
	 ...
	 if "wxt520" in song_checker_config.rain_detector and rain_event == 0:
		if len(numpy.nonzero(wxt520_rain_arr)[0]) > song_checker_config.max_rain_events and float(weather_output["wxt520_humidity"]) > 20.:	##### Only when humidity is above 20 percent #####
...
############### When WXT520 is working this should be used:
      if round(float(numpy.mean(wind_avg_arr)),3) == round(float(wind_avg_arr[0]),3) and round(float(numpy.mean(wind_avg_arr)),3) == round(float(wind_avg_arr[-1]),3) and round(float(numpy.mean(wind_speed_arr)),3) == round(float(wind_speed_arr[0]),3) and round(float(numpy.mean(wind_speed_arr)),3) == round(float(wind_speed_arr[-1]),3):
   ...
   def object_check(self, object_ra, object_dec, site=site_param):
	...
	try:
		...
		curr.close()
		conn.close()
#	if len(numpy.where(numpy.array(wind_data["wxt520_wind_speed"],dtype=float) >= song_checker_config.max_w_speed_into)[0]) >= song_checker_config.wind_blow_events and numpy.abs(object_az - float(wind_data["wxt520_wind_avgdir"][-1])) < song_checker_config.angle_into:
#		object_value = object_value + 2     
   ...
   def check_obs_state(self):
	...
	if str(mirror_cover_state) in ['0', '0.0']:
	...
	if str(motion_state) not in ['11', '11.0', '1', '1.0']:
		if float(tel_dome_output["tel_alt"]) <= float(m_conf.telescope_min_altitude) - 1.0:
			return 7
		else:
			return 4
...
#####################################################################################
	...
	try:
		...
		ins_at_tenerife = datetime.datetime.strptime(str(or_tenerife_output["ins_at"]), "%Y-%m-%d %H:%M:%S.%f")
	...
	try:
		...
		ins_at_central = datetime.datetime.strptime(str(or_central_output["ins_at"]), "%Y-%m-%d %H:%M:%S.%f")
...
#####################################################################################
	...
	if weather_id_tenerife != weather_id_central and weather_ins_at_central < (datetime.datetime.utcnow() - datetime.timedelta(seconds=600)):
		return_value = return_value + 2, weather_ins_at_central
   ...
   def check_last_thar_spectrum(self):
	"""
		@brief: This function checks when last thar spectrum was acquired.
		Returns 1 if a new ThAr should be acquired or 0 if not.
	"""
	try:
		conn = psycopg2.connect("host='%s' dbname='%s' user='%s' password='%s'" % (m_conf.db_host, m_conf.data_db, m_conf.db_user, m_conf.db_password))
		curr = conn.cursor()
		curr.execute("SELECT file_name FROM spectrograph_cam WHERE ins_at > '%s' AND param_1 = 3" % (datetime.datetime.utcnow() - datetime.timedelta(seconds=300)))	# param_1 = 3 is ThAr
		spec_cam_output = curr.fetchone()
	...
	if rowcount > 0:
		print clock.timename(), " ThAr spectrum close enough in time: -", spec_cam_output[0], "- no need for one more..."
		return 0
	else:
		return 1

   def check_last_observed_target(self, next_object=""):
	"""
		@brief: This function will check which object or OR was observed during the last exposure
	"""
	
	### Find the request number of the last observed spectrum:
	spec_data = ""
	try:
		conn = psycopg2.connect("host='%s' dbname='%s' user='%s' password='%s'" % (m_conf.db_host, m_conf.data_db, m_conf.db_user, m_conf.db_password))
		curr = conn.cursor()
		spec_data = self.get_fields(curr, "spectrograph_cam", fields=["req_no"])
	except Exception, e:
		print clock.timename(), " Error1: ", e		
        else:
	...
	if spec_data != "":
		if int(spec_data["req_no"]) != 0:
			print clock.timename(), " Checking last observed spectrum from OR: %i" % (spec_data["req_no"])

			try:
				conn = psycopg2.connect("host='%s' dbname='%s' user='%s' password='%s'" % (m_conf.db_host, m_conf.or_db, m_conf.db_user, m_conf.db_password))
				curr = conn.cursor()
				OR_data = self.get_fields_req_no(curr, "obs_request_1", fields=["obs_mode", "object_name", "right_ascension", "declination", "magnitude"], req_no=spec_data["req_no"])
			except Exception, e:
				print clock.timename(), " Error2: ", e		
			else:
				if next_object != "":
					### Collect next OR data:
					try:
						conn = psycopg2.connect("host='%s' dbname='%s' user='%s' password='%s'" % (m_conf.db_host, m_conf.or_db, m_conf.db_user, m_conf.db_password))
						curr = conn.cursor()
						next_OR_data = self.get_fields_req_no(curr, "obs_request_1", fields=["obs_mode", "object_name", "right_ascension", "declination", "magnitude"], req_no=next_object)
					except Exception, e:
						print clock.timename(), " Error3: ", e				
			
					### Check if the object parameters are the same:
					if OR_data["object_name"] == next_OR_data["object_name"] or (OR_data["right_ascension"] == next_OR_data["right_ascension"] and OR_data["declination"] == next_OR_data["declination"]):
						print clock.timename(), " Next object to be observed is the same as last and acquisition is not required again!"
					
						return "same"
					else:					
						print clock.timename(), " Next object is NOT the same as the last!"
					
						return "not_same"

	return "not_same"

   def check_last_solved_skycam_im(self):

	try:
		conn = psycopg2.connect("host='%s' dbname='%s' user='%s' password='%s'" % (m_conf.db_host, m_conf.data_db, m_conf.db_user, m_conf.db_password))
		curr = conn.cursor()
		curr.execute("SELECT numb_stars, ins_at FROM star_tracker WHERE star_tracker_id = (SELECT max(star_tracker_id) FROM star_tracker)") # WHERE im_solved=1)")
		skycam_output = curr.fetchone()
		curr.close()
		conn.close()
	except Exception, e:
		print clock.timename(), " Error: ", e
