...
if len(sys.argv) > 1:
    if sys.argv[1] == '-h' or sys.argv[1] == '--help':
       ...
       if int(mode) not in [0,1]:
          print '  The mode value must be 0,1 [high,low]'
          sys.exit("  Wrong value for state!")
else:
    print '  Please specify VS speed'
    sys.exit("  No mode has been given!")

print 'Changing the VS speed...'
try:
   print server.set_vss(int(mode))
except Exception ,e:
   print 'Could not connect to the camera.'
   print e
   
if e == None:
  print 'VS speed has changed!'
