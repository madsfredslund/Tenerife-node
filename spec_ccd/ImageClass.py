...
class imClass(object):
   ...
   """    
   def acquire_one_image(self,cam,filename,req_no,exptime,hsspeed,pregain,highcap,hbin,vbin,acmode,hstart,hend,vstart,vend,imagetyp,object_name, obj_ra, obj_dec, comment):
      """
      ...
      if cam.GetTemperature() == 'DRV_NOT_INITIALIZED':
         ...
         return_value = self.take_sci_im(cam,filename,req_no,exptime,hsspeed,pregain,highcap,hbin,vbin,acmode,hstart,hend,vstart,vend,imagetyp,object_name, obj_ra, obj_dec, comment)
         if return_value == 0:
   ...
   def take_sci_im(self,cam,filename,req_no,exptime,hsspeed,pregain,highcap,hbin,vbin,acmode,hstart,hend,vstart,vend,imagetyp,object_name, obj_ra, obj_dec, comment):
      """ 
...
#            print "finished waiting: ",clock.whattime()  
		...
		data_type = 0
                value = cam.SaveAsFITS(filename, data_type)
	     #  value = cam.SaveAsFITS_TEST(filename)	# Works on sstenerife but is slow...
            if value == "DRV_ACQUIRING":
               cam.AbortAcquisition()
               print "Image was skiped!"
	...
	###### WHEN WE START USING req_no the insertion string below should be modified.....##############
	 ...
	 try:
         	ret_val = mh.makeheader(cam,ccdtemp,date_obs,jd_obs,mjd_obs,filename,hsspeed,pregain,highcap,imagetyp,object_name,req_no, obj_ra, obj_dec, comment, used_exp_time)
		#print "make header disabled"
	 except Exception,e:
		print clock.timename(), "Could not update the header..."
		print clock.timename(), e 
		ret_val = 0
	 if ret_val != 1:
		send_song_mail.send_mail().sending_an_email(reciever=m_conf.notify_email_who,sender="SONG_MS",subject="Header of spectrum was not created!",message="The header of spectrum:\n%s\nwas not updated with the correct parameters. This is an issue for the pipeline.\n\nSend at: %s\n\n" % (filename, clock.obstimeUT()))
	 else:
