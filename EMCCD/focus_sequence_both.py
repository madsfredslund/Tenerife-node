...
sys.path.append("/home/madsfa/subversion/trunk/phot/red/")
sys.path.append("/home/madsfa/subversion/trunk/phot/vis/")
import master_config as m_conf
import red_ccd_config as red_conf
...
red_server = xmlrpclib.ServerProxy('http://%s:%s' % (red_conf.serverhost, red_conf.port))
...
parser.add_option("-r", "--rocus_range", type="float", dest="range", default=0.1, help="Set focus range to check: ex. -r0.2")
...
try:
	value = red_server.acquire_an_image_thread("test.fits", "", 0.1, hsspeed, pregain, highcap, hbin, vbin, acmode, "", "", "", "", "star", "", "", "", "", "", "red")
except Exception, e:
	print 'error'
else:
...
for step in range(number_of_steps):
	...
	red_filename = "red_focus_%i.fits" % (step+1)
	vis_filename = "vis_focus_%i.fits" % (step+1)
	 
	try:
		value_red = red_server.acquire_an_image_thread(image_dir+red_filename, "", exptime, hsspeed, pregain, highcap, hbin, vbin, acmode, "", "", "", "", "star", "", "", "", "", "focus", "red")
	except Exception, e:
		print 'error'
	try:
...
print "Calculate best forcus for RED CCD"
best_focus_red = calculate_best_focus(image_dir, red_images, "red")

print "Done determining focus..."
print "Best focus for vis: ", best_focus_vis
print "Best focus for red: ", best_focus_red
