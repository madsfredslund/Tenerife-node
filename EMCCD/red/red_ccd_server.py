...
def initialize_cam():
   ...
   try:
	...
	print cam.SetImageFlip(0,1)	# Flip the image in vertical direction to match the visual CCD image
   except Exception, e:
      print 'error: ', e
      stamp = 1
   if stamp != 1:
...
def Spool(number, target, emgain, exptime):
        ...
        *exposureTime* is set to *andorsetup.expTime[0]*'''
        ...
        for i in range(6):
            try:
                fitsFile = pyfits.open(andorsetup.calibrationDirectory["red"]+'flat_'+str(i+1)+'.fits')[0]
                flat[i] = fitsFile.data
                flatConfiguration[i] = fitsFile.header
            except:
                print('No flat '+str(i)+' frame found, using default')
        try:
            fitsFile = pyfits.open(andorsetup.calibrationDirectory["red"]+'bias.fits')[0]
            bias = fitsFile.data
            biasConfiguration = fitsFile.header
        except:
            print('Found no bias frame, using default, that is zero (pure offset)')
            bias = numpy.zeros((512,530))
            biasConfiguration = {}
        try:
            fitsFile = pyfits.open(andorsetup.calibrationDirectory["red"]+'spurious.fits')[0]
            spurious = fitsFile.data
            spuriousConfiguration = fitsFile.header
        except:
            print('Found no spurious map, using default, that is zero.')
            spurious = numpy.zeros((512,530)) # Replace with real code
            spuriousConfiguration = {}
        try:
            fitsFile = pyfits.open(andorsetup.calibrationDirectory["red"]+'flatbias.fits')[0]
	...
	cam.GetStatus()
     ...
     #       try:
		...
		print "number of exposures: ", nExposure
...
def acquire_an_image(filename,req_no,exptime,hsspeed,pregain,highcap,hbin,vbin,acmode,hstart,hend,vstart,vend,imagetyp,object_name, obj_ra, obj_dec, DayNight_flag, comment, camera="red"):
   """
...
def acquire_an_image_thread(filename,req_no,exptime,hsspeed,pregain,highcap,hbin,vbin,acmode,hstart,hend,vstart,vend,imagetyp,object_name, obj_ra, obj_dec, DayNight_flag, comment, camera="red"):
	"""
		@brief: This function will acquire an image and save it as a FITS file. 
...
def check_filename(filename, IMAGE_DIR_PATH):
   ...
   if filename != "mfa.fits":
	...
	filename = "r%s_%s.fits" % (conf.site, time.strftime("%Y-%m-%dT%H-%M-%S", time.localtime()))
	image_path = "%s/%s" % (IMAGE_DIR_PATH, filename)
...
class cam_server(Daemon):
  ...
  #    val = beating_heart.start_heartbeat(job_id=m_conf.spec_ccd_id)
      ...
      stamp = 0
      try:
         server = SimpleXMLRPCServer((conf.serverhost, conf.port), requestHandler=RequestHandler, logRequests=False)
      except Exception, e:
         print 'error: ', e
         stamp = 1
      if stamp != 1:
...
def main():
   ...
   daemon = cam_server(conf.pidfile, stdout=conf.outstream, stderr=conf.outstream)
   try:
   ...
   for opt, a in opts:
      if opt == "-s":
         ...
         import xmlrpclib
         server = xmlrpclib.ServerProxy('http://'+str(conf.serverhost)+':'+str(conf.port))
