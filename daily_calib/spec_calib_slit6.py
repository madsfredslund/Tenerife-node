...
slitpos          = 6               # allowed values are: 8,7,6,5,4,3,2,1
...
exp_time_thar    = 2.7             # The exposure time in seconds.
exp_time_flat    = 2.5             # The exposure time in seconds.
exp_time_flati2  = 2.5             # The exposure time in seconds.
...
os.system( SLIT_PATH + "/sigu.py stop")          # Do not enable the guiding
...
time.sleep(5)
Set_M8.set_m8_pos()
#os.system( SLIT_PATH + "/sigu.py stop")          # stop showing the image and stop the guide loop
...
slit_location_im = 1
sigu_snap_filename = "/scratch/star_spec/%s/guide_images/slit6_halo_slit_in.fits" % (folder_date)
...
if slit_location_im == 1:
	os.system( "%s/pst.py enc -m6 -p%s" % (DMC_PATH, 900000) )     # Move slit out of halo 
	time.sleep(6)	# Make sure a new snapshot is taken

	sigu_snap_filename = "/scratch/star_spec/%s/guide_images/slit6_halo_slit_out.fits" % (folder_date)
	os.system( SLIT_PATH + "/sigu.py snap %s" % sigu_snap_filename)       # Save snapshot of guide image
