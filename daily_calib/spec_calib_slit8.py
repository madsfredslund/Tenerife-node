...
#   21022014: Changed to ONLY make flat fields and thar for slit 8 - no bias.
#             Use 'spec_calib2.py' for general use and slit 6!
##############################################################################
#
...
slitpos          = 8               # allowed values are: 8,7,6,5,4,3,2,1
...
exp_time_thar    = 4.0             # The exposure time in seconds.
exp_time_flat    = 3.6             # The exposure time in seconds.
exp_time_flati2  = 3.6             # The exposure time in seconds.
...
try:
	os.system( "%s/c_acq.py -p%i -r%i -e%f -t%s --daynight=%s -fdaily_thar_1.fits" % (ANDOR_PATH, pre_amp_gain, readoutspeed, 1.0, 'CHECK', 'day' ) )
	os.system( "%s/c_acq.py -p%i -r%i -e%f -t%s --daynight=%s -fdaily_thar_2.fits" % (ANDOR_PATH, pre_amp_gain, readoutspeed, 1.0, 'CHECK', 'day' ) )
except Exception,e:
	print e
...
slit_location_im = 1
sigu_snap_filename = "/scratch/star_spec/%s/guide_images/slit8_halo_slit_in.fits" % (folder_date)
...
if slit_location_im == 1:
	os.system( "%s/pst.py enc -m6 -p%s" % (DMC_PATH, 1200000) )     # Move slit out of halo 
	time.sleep(6)	# Make sure a new snapshot is taken

	sigu_snap_filename = "/scratch/star_spec/%s/guide_images/slit8_halo_slit_out.fits" % (folder_date)
	os.system( SLIT_PATH + "/sigu.py snap %s" % sigu_snap_filename)       # Save snapshot of guide image
